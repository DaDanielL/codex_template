...
description: Use when a product or feature idea needs guided discovery questions before Codex writes a PRD.
---
...
{user research, technical spike, or `rules-interactive {prd-path}` when the project structure is unknown; otherwise `create-stories {prd-path}`}
```
